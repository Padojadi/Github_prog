...
import { DomesticAndRelativeDCCreationAttributes } from '../../database/models/card/domesticAndRelative/domesticandrelativedc';
...
/**
 * Factory for creating DomesticAndRelativeDC test data
 */
export class DomesticAndRelativeDCFactory {
  /**
   * Build minimal DomesticAndRelativeDC data (only required fields)
   */
  static buildMinimal(
    creatorId: string,
    organismId: string,
    ownerDiplomaticCardId: string,
    overrides: Partial<DomesticAndRelativeDCCreationAttributes> = {}
  ): DomesticAndRelativeDCCreationAttributes {
  ...
  /**
   * Build printed DomesticAndRelativeDC
   */
  static buildPrinted(
    creatorId: string,
    organismId: string,
    ...
    return this.buildMinimal(creatorId, organismId, ownerDiplomaticCardId, {
      ...
      cardNumber: `DR-${faker.string.numeric(4)}-${faker.string.numeric(3)}-${new Date().getFullYear()}`,
