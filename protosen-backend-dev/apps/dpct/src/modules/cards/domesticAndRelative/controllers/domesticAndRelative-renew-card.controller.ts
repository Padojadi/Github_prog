...
import { GetDomesticAndRelativesDCInput } from '../dtos/domesticAndRelative-card.dto';
...
class RenewDomesticAndRelativeDCController {
	async getDomesticAndRelativeCard(req: Request<GetByIdInput>, res: Response) {
		try {
			const { id } = req.params;
			const userAuth = res.locals.userAuth;

			const card = await authorizationService.getAuthorizedResource(
				new RenewDomesticAndRelativeDCService(),
				id,
				userAuth,
			);
	...
	async createRenewal(req: Request, res: Response) {
		try {
			...
			const cardToRenew = await new DomesticAndRelativeDCService().getById(id);
			if (!cardToRenew) {
				return res.status(HttpStatusCode.NotFound).json({
					message: ERROR_MESSAGE.NOT_FOUND_MESSAGE,
				});
			...
			const newCard = await new RenewDomesticAndRelativeDCService().save(payload);
	...
	async getDomesticAndRelativesDC(
		req: Request<{}, {}, {}, GetDomesticAndRelativesDCInput>,
		res: Response,
	) {
		try {
			const {
			...
			const response = await new RenewDomesticAndRelativeDCService().getEntities(
	...
	async pointFocalValidation(req: Request<GetByIdInput>, res: Response) {
		try {
			...
			const card = await new RenewDomesticAndRelativeDCService().getOne({
				id: id,
				organismId: currentUser.organismId,
				creatorId: currentUser.id,
			});
			...
			const updatedDocument = await new RenewDomesticAndRelativeDCService().update(id, {
	...
	async adminValidation(
		...
		try {
			...
			const card = await new RenewDomesticAndRelativeDCService().getById(id);
			if (!card)
				{return res.status(HttpStatusCode.NotFound).json({ message: 'Carte non trouvable' });}
			...
			const updatedDocument = await new RenewDomesticAndRelativeDCService().update(id, data);
	...
	async superAdminValidation(
		...
		try {
			...
			if (data.documentStage === ESuperAdminDocumentState.CONFIRMED) {
				await new DomesticAndRelativeDCService().update(previousCard.id, { expired: true });
				payload = { ...data, newDateEndOfMission: validity };
			} else {
				payload = data;
			}

			const updatedDocument = await new RenewDomesticAndRelativeDCService().update(id, payload);
	...
	async setPrintedRenewDomesticAndRelativeDC(
		req: Request<
			{ id: string },
			{},
			{
	...
	async undoPrintRenewDomesticAndRelativeDC(req: Request<{ id: string }, {}, {}>, res: Response) {
		try {
			const { id } = req.params;
	...
	async setReturned(req: Request<GetByIdInput>, res: Response) {
		try {
			...
			const updatedCard = await new RenewDomesticAndRelativeDCService().update(id, {
...
export default new RenewDomesticAndRelativeDCController();
