...
import { AttachFileDomesticAndRelativeDCInput, DeleteFileInput } from '@shared/schemas/file.schema';
import {
	CreateDomesticAndRelativeDCInput,
	DomesticAndRelativeDCbYIdInput,
	GetDomesticAndRelativesDCInput,
	PrintDomesticAndRelativeDCSchema,
	UpdateDomesticAndRelativeDCInput,
} from '../dtos/domesticAndRelative-card.dto';
import { DomesticAndRelativeDCService } from '../services/domesticAndRelative-card.service';
import { DomesticAndRelativeDCFileService } from '../services/domesticAndRelative-card-file.service';
...
class DomesticAndRelativeDCController {
	async create(req: Request<{}, {}, CreateDomesticAndRelativeDCInput>, res: Response) {
		try {
			const data = req.body;
			...
			const newOwnerCard = await new DomesticAndRelativeDCService().save({
	...
	async deleteDomesticAndRelativeDC(req: Request, res: Response) {
		try {
			const { id } = req.params;
			const userAuth = res.locals.userAuth;

			const card = await authorizationService.getAuthorizedResource(
				new DomesticAndRelativeDCService(),
				id,
				userAuth,
			);

			const domesticAndRelativeDCService = new DomesticAndRelativeDCService();
			domesticAndRelativeDCService.validateCanDelete(card);

			// Récupérer ses fichiers associés
			const files = await new DomesticAndRelativeDCFileService().getAll({
				where: { domesticAndRelativeDCId: id },
			});

			await new DomesticAndRelativeDCService().delete(id);
	...
	async getById(req: Request<DomesticAndRelativeDCbYIdInput>, res: Response) {
		try {
			const { id } = req.params;
			const userAuth = res.locals.userAuth;
			...
			if (card.domesticAndRelativeDCFiles) {
				const fileService = new FileService();
				const { passportKey, adKey, photoKey, othersKey } = card.domesticAndRelativeDCFiles;
	...
	async attachFiles(req: Request<{}, {}, AttachFileDomesticAndRelativeDCInput>, res: Response) {
		try {
			const { domesticAndRelativeDCId } = req.body;
			const userAuth = res.locals.userAuth;
			...
			if (card.domesticAndRelativeDCFiles && card.domesticAndRelativeDCFiles.id) {
			...
			const newAttachedFile = await new DomesticAndRelativeDCFileService().save({
				domesticAndRelativeDCId: domesticAndRelativeDCId,
	...
	async getList(req: Request<{}, {}, {}, GetDomesticAndRelativesDCInput>, res: Response) {
		try {
			const {
				status,
				search,
			...
			const response = await new DomesticAndRelativeDCService().getAll(
	...
	async update(
		req: Request<
			UpdateDomesticAndRelativeDCInput['params'],
			{},
			UpdateDomesticAndRelativeDCInput['body']
		>,
		res: Response,
	) {
		try {
			...
			const domesticAndRelativeDCService = new DomesticAndRelativeDCService();
			domesticAndRelativeDCService.validateCanUpdate(card, userAuth.role);

			const updatedDocument = await new DomesticAndRelativeDCService().update(id, data);
	...
	async attachOtherFiles(
		req: Request<{}, {}, AttachFileDomesticAndRelativeDCInput>,
		res: Response,
	) {
		try {
			const { domesticAndRelativeDCId } = req.body;
			...
			const domesticAndRelativeFiles = await new DomesticAndRelativeDCFileRepo().findById(
				card.domesticAndRelativeDCFiles.id,
			);

			if (!domesticAndRelativeFiles) {
				return res.status(HttpStatusCode.NotFound).json({
					message: ERROR_MESSAGE.NOT_FOUND_MESSAGE,
				});
			}

			let photoKey = domesticAndRelativeFiles.photoKey;
			let passportKey = domesticAndRelativeFiles.passportKey;
			let adKey = domesticAndRelativeFiles.adKey;
			...
			if ('passport' in req.files) {
				if (domesticAndRelativeFiles.passportKey) {
					deletePromises.push(deleteFile(domesticAndRelativeFiles.passportKey));
				}
			...
			if ('ad' in req.files) {
				if (domesticAndRelativeFiles.adKey) {
					deletePromises.push(deleteFile(domesticAndRelativeFiles.adKey));
				}
			...
			if ('photo' in req.files) {
				if (domesticAndRelativeFiles.photoKey) {
					deletePromises.push(deleteFile(domesticAndRelativeFiles.photoKey));
				}
			...
			let updatedOthersKey = domesticAndRelativeFiles.othersKey;
			if (uploadedOtherFileKeys.length > 0) {
				const existingOtherFiles = domesticAndRelativeFiles.othersKey ?? [];
				updatedOthersKey = [...existingOtherFiles, ...uploadedOtherFileKeys];
			}

			await new DomesticAndRelativeDCFileRepo().update(domesticAndRelativeFiles.id, {
				passportKey,
				adKey,
				photoKey,
	...
	async pointFocalValidation(req: Request<GetByIdInput>, res: Response) {
		try {
			...
			const card = await new DomesticAndRelativeDCService().getOne({
				id: id,
				organismId: currentUser.organismId,
			...
			const updatedDocument = await new DomesticAndRelativeDCService().update(id, {
	...
	async setPrintDomesticAndRelativeDC(
		req: Request<
			PrintDomesticAndRelativeDCSchema['params'],
			{},
			PrintDomesticAndRelativeDCSchema['body']
		>,
		res: Response,
	) {
		try {
			...
			const domesticAndRelativeDCService = new DomesticAndRelativeDCService();
			domesticAndRelativeDCService.validateCanPrint(card);
	...
	async undoPrintDomesticAndRelativeDCC(
		req: Request<PrintDomesticAndRelativeDCSchema['params'], {}, {}>,
		res: Response,
	) {
		try {
			const { id } = req.params;

			const card = await new DomesticAndRelativeDCService().getById(id);
			...
			const domesticAndRelativeDCService = new DomesticAndRelativeDCService();
			domesticAndRelativeDCService.validateCanUndoPrint(card);
	...
	async setReturned(req: Request<GetByIdInput>, res: Response) {
		try {
			...
			const updatedCard = await new DomesticAndRelativeDCService().update(id, {
...
export default new DomesticAndRelativeDCController();
