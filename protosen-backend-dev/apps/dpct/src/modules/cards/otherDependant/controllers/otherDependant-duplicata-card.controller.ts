...
import { OtherDependantDuplicataDCService } from '../services/otherDependant-duplicata.service';
...
class OtherDependantDuplicataDCController {
	async demandDuplicataCard(req: Request<{}, {}, CreateDuplicataDCInput>, res: Response) {
		try {
			...
			const newDuplicata = await new OtherDependantDuplicataDCService().save({
	...
	async getOtherDependantsDuplicataDC(
		...
		try {
			...
			const response = await new OtherDependantDuplicataDCService().getEntities(
	...
	async getOtherDependantDuplicataDC(req: Request<GetByIdInput>, res: Response) {
		try {
			const { id } = req.params;
			const userAuth = res.locals.userAuth;

			const card = await authorizationService.getAuthorizedResource(
				new OtherDependantDuplicataDCService(),
				id,
				userAuth,
			);
	...
	async pointFocalValidation(req: Request<GetByIdInput>, res: Response) {
		try {
			...
			const card = await new OtherDependantDuplicataDCService().getOne({
				where: { id: id },
			});
			...
			const updatedDocument = await new OtherDependantDuplicataDCService().update(id, {
	...
	async adminValidation(
		...
		try {
			...
			const card = await new OtherDependantDuplicataDCService().getById(id);
			...
			const updatedDocument = await new OtherDependantDuplicataDCService().update(id, data);
	...
	async setPrintedOtherDependantDuplicataDC(
		req: Request<
			PrintOtherDependantDuplicataDCSchema['params'],
			{},
			PrintOtherDependantDuplicataDCSchema['body']
		>,
		res: Response,
	) {
		try {
			...
			new OtherDependantDuplicataDCService().validateCanPrint(card);

			const updatedCard = await new OtherDependantDuplicataDCService().update(id, {
	...
	async undoPrintOtherDependantDuplicataDC(req: Request<GetByIdInput>, res: Response) {
		try {
			const { id } = req.params;
			const userAuth = res.locals.userAuth;
			...
			new OtherDependantDuplicataDCService().validateCanUndoPrint(card);
...
export default new OtherDependantDuplicataDCController();
