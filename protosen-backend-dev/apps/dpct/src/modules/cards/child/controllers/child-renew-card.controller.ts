...
import { GetChildsDCInput } from '../dtos/child-card.dto';
...
class RenewChildDCController {
	async createRenewal(req: Request, res: Response) {
		try {
			...
			const cardToRenew = await new ChildDCService().getById(id);
			if (!cardToRenew) {
				return res.status(HttpStatusCode.NotFound).json({
			...
			const newOwnerCard = await new RenewChildDCService().save(payload);
	...
	async getChildsDC(req: Request<{}, {}, {}, GetChildsDCInput>, res: Response) {
		try {
			...
			const response = await new RenewChildDCService().getEntities(
	...
	async pointFocalValidation(req: Request<GetByIdInput>, res: Response) {
		try {
			...
			const card = await new RenewChildDCService().getOne({ id });

			if (!card)
				{return res.status(HttpStatusCode.NotFound).json({ message: 'Carte non trouvable' });}

			// Verify organismId and creatorId on the previousCard
			if (
				card.previousCard?.organismId !== currentUser.organismId ||
				card.previousCard?.creatorId !== currentUser.id
			) {
				return res.status(HttpStatusCode.Forbidden).json({
					message: "Vous n'êtes pas autorisé à accéder à cette carte",
				});
			}
			...
			const updatedDocument = await new RenewChildDCService().update(id, {
	...
	async adminValidation(
		...
		try {
			...
			const card = await new RenewChildDCService().getById(id);
			if (!card)
				{return res.status(HttpStatusCode.NotFound).json({ message: 'Carte non trouvable' });}
			...
			const updatedDocument = await new RenewChildDCService().update(id, data);
	...
	async superAdminValidation(
		...
		try {
			...
			if (data.documentStage === ESuperAdminDocumentState.CONFIRMED) {
				await new ChildDCService().update(previousCard.id, { expired: true });
				payload = { ...data, newDateEndOfMission: validity };
			} else {
				payload = data;
			}

			const updatedDocument = await new RenewChildDCService().update(id, payload);
	...
	async undoPrintRenewChildDC(req: Request<{ id: string }, {}, {}>, res: Response) {
		try {
			const { id } = req.params;
	...
	async setReturned(req: Request<GetByIdInput>, res: Response) {
		try {
			...
			const updatedCard = await new RenewChildDCService().update(id, {
...
export default new RenewChildDCController();
