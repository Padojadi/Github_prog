...
import { GetSpousesDCInput } from '../dtos/spouse-card.dto';
...
class RenewSpouseDCController {
	async getSpouseCard(req: Request<GetByIdInput>, res: Response) {
		try {
			const { id } = req.params;
			const userAuth = res.locals.userAuth;

			const card = await authorizationService.getAuthorizedResource(
				new RenewSpouseDCService(),
				id,
				userAuth,
			);
	...
	async createRenewal(req: Request, res: Response) {
		try {
			...
			const cardToRenew = await new SpouseDCService().getById(id);
			if (!cardToRenew) {
				return res.status(HttpStatusCode.NotFound).json({
					message: ERROR_MESSAGE.NOT_FOUND_MESSAGE,
				});
			...
			const newOwnerCard = await new RenewSpouseDCService().save(payload);
	...
	async getSpousesDC(req: Request<{}, {}, {}, GetSpousesDCInput>, res: Response) {
		try {
			...
			const response = await new RenewSpouseDCService().getEntities(
	...
	async pointFocalValidation(req: Request<GetByIdInput>, res: Response) {
		try {
			...
			const card = await new RenewSpouseDCService().getOne({
				id: id,
				organismId: currentUser.organismId,
				creatorId: currentUser.id,
			});
			...
			const updatedDocument = await new RenewSpouseDCService().update(id, {
	...
	async adminValidation(
		...
		try {
			...
			const card = await new RenewSpouseDCService().getById(id);
			if (!card)
				{return res.status(HttpStatusCode.NotFound).json({ message: 'Carte non trouvable' });}
			...
			const updatedDocument = await new RenewSpouseDCService().update(id, data);
	...
	async superAdminValidation(
		...
		try {
			...
			if (data.documentStage === ESuperAdminDocumentState.CONFIRMED) {
				await new SpouseDCService().update(previousCard.id, { expired: true });
				payload = { ...data, newDateEndOfMission: validity };
			} else {
				payload = data;
			}

			const updatedDocument = await new RenewSpouseDCService().update(id, payload);
	...
	async undoPrintRenewSpouseDC(req: Request<{ id: string }, {}, {}>, res: Response) {
		try {
			const { id } = req.params;
	...
	async setReturned(req: Request<GetByIdInput>, res: Response) {
		try {
			...
			const updatedCard = await new RenewSpouseDCService().update(id, {
...
export default new RenewSpouseDCController();
