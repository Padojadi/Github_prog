...
import { updateVisaRequestStatus } from "@/features/visas/lib/server-workflow-store";
import type { VisaWorkflowAction } from "@/features/visas/lib/workflow-types";
...
export async function PATCH(
	...
	let body: { action?: VisaWorkflowAction; reason?: string };
	try {
		body = (await request.json()) as { action?: VisaWorkflowAction; reason?: string };
	} catch {
	...
	try {
		const updated = await updateVisaRequestStatus(id, body.action, body.reason, actor);
