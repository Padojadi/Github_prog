...
import { markCardAsPrinted } from "@/lib/actions/diplomaticCards/childs";
...
export default function TabLayout({ child }: { child: any }) {
	const currentUser = useCurrentUser();
	const router = useRouter();

	const canPreviewCard =
		(child?.documentStage === "accepted" ||
			child?.documentStage === "confirmed" ||
			child?.documentStage === "printed") &&
		currentUser?.role !== "user";
	const canPreviewDocument =
		child?.documentStage === "confirmed" || child?.documentStage === "printed";
			...
			<Tabs defaultValue="requestFormfile">
				...
				<TabsContent value="idcard">
					<IdCardSection
						...
						url={"/panel/diplomatic/childs/" + child.id + "/pdf/id-card"}
					/>
				</TabsContent>
