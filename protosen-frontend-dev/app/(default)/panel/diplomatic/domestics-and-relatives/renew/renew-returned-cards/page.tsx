...
import {
	fetchDomesticsAndRelativesReturnedCardsRenew,
} from "@/lib/actions/diplomaticCards/domesticAndRelatives";
import type { IHolder } from "@/lib/types";

export default async function Page() {
	const res = await fetchDomesticsAndRelativesReturnedCardsRenew();
	...
	return (
		<div>
			<TableComponentReturned
				headers={headers}
				title="Cartes renouvelées restituées personnels de services, domestiques et familles"
				data={Array.isArray(data) ? data : []}
				actions={[
					{
						label: "details",
						href: "/panel/diplomatic/domestics-and-relatives/renew/renew-returned-cards",
					},
				]}
			/>
		</div>
