...
import {
	fetchOtherStaffsReturnedCardsRenew,
} from "@/lib/actions/diplomaticCards/otherStaffs";
import type { IHolder } from "@/lib/types";

export default async function Page() {
	const res = await fetchOtherStaffsReturnedCardsRenew();
	...
	return (
		<div>
			<TableComponentReturned
				headers={headers}
				title="Cartes renouvelées restituées autres personnels"
				data={Array.isArray(data) ? data : []}
				actions={[
					{
						label: "details",
						href: "/panel/diplomatic/other-staff/renew/renew-returned-cards",
					},
				]}
			/>
		</div>
