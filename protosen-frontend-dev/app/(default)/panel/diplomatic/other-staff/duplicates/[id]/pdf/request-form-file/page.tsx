...
export default async function Page({
	...
	if (canPrint === false) {
		redirect("/panel/diplomatic/other-staff/duplicates");
	}

	return (
		<>
			<Link
				href={`/panel/diplomatic/other-staff/duplicates/${id}?previousCard=${previousCardId}`}
				className="btn mr-3 bg-indigo-500 hover:bg-indigo-600 text-white"
			>
				<BsChevronLeft />
				<span className="ml-2">Retour</span>
			</Link>
			<MyPDFPreview otherStaff={otherStaff} />
		</>
	);
}
