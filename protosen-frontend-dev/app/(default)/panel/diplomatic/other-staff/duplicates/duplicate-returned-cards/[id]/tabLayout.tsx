...
export default function TabLayout({
	...
	otherStaff: any;
	otherStaffDuplicate: any;
}) {
	const currentUser = useCurrentUser();
	const router = useRouter();

	const canPreviewCard =
		otherStaffDuplicate?.documentStage === "RETURNED" &&
		currentUser?.role !== "user";
	const canPreviewDocument = otherStaffDuplicate?.documentStage === "RETURNED";
			...
			<Tabs defaultValue="requestFormfile">
				...
				<TabsContent value="idcard">
					<IdCardSectionDuplicateReturned
						person={otherStaff}
						personDuplicate={otherStaffDuplicate}
					/>
				</TabsContent>
