...
const ValidateForm = ({
	...
	useEffect(() => {
		...
		async function fetchRenewData() {
			if (renewId) {
				const res = await fetchRenewOtherDependantCardById(renewId);
				if (res.status === "error") {
		...
		async function fetchDuplicateData() {
			if (duplicateId) {
				const res = await fetchDuplicateOtherDependantCardById(duplicateId);
				if (res.status === "error") {
					throw new Error(res.message);
				}
								...
								<li className="flex flex-col">
									<div className="flex items-center">
										...
										<span className="flex-1">
											{person.otherDependantDCFiles.othersKey.length} Autre(s)
											Fichier(s)
										</span>
									</div>
