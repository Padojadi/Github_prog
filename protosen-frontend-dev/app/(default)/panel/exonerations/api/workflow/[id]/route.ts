...
import { updateExonerationRequestStatus } from "@/features/exonerations/lib/server-workflow-store";
import type { ExonerationWorkflowAction } from "@/features/exonerations/lib/workflow-types";
...
export async function PATCH(
	...
	let body: { action?: ExonerationWorkflowAction; reason?: string };
	try {
	...
	try {
		const updated = await updateExonerationRequestStatus(
			id,
			body.action,
			body.reason,
			actor
