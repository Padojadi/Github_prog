...
import IdCardFont from "./idCardFont";
...
const MyPDFPreview = dynamic(
	() =>
		import("@/app/(default)/panel/diplomatic/holders/[id]/pdf/id-card/myPDFPreview"),
	{ ssr: false },
);
...
export default function IdCardSection({
	...
	// const [renewData, setRenewData] = React.useState<any>();
	...
	const confirmed = person?.documentStage === "confirmed";

	const printed = person?.documentStage === "printed";
	...
	//   return node.id !== "photoLink";
	// }
	// console.log(person);
	// const downloadImgJpeg = (id: string, fileName: string) => {
	//   const node = document.getElementById(id);
	//   if (node) {
	//     toJpeg(node, { quality: 0.95, filter: filter }).then(function (dataUrl) {
	...
	const markAsPrinted = async () => {
		// console.log(person?.deliverThe, person?.validUntil);
		// console.log(deliverDate, expirationDate);
		const data = {
		...
		setLoading(true);
		const res = await updateFn(data, person?.id);
		// console.log(res);
		if (res?.status === "error") {
	...
	useEffect(() => {
		if (person && data && plateData) {
			const entity = data?.data?.find(
				(e: CardType) => e.name === person?.type_card,
			);
			const plaqueChosed = plateData?.data?.find(
				(e: TPlate) => e.code === person?.plaque,
			);
			setCardTitle(person?.type_card || "");
			setDeliverDate(
				person?.issueDate && person?.issueDate.length > 0
					? person.issueDate
					: new Date().toISOString().split("T")[0],
			);
			setExpirationDate(person?.validUntil);
			setPrintColor(person?.color || "#AEFCAC");
			if (entity) {
		...
		<>
			<ResponsiveModal open={openPrintModal}>
				<ResponsiveModalContent className="lg:max-w-6xl [&>button]:hidden overflow-hidden">
					...
					<div className="h-[calc(70%-128px)] overflow-auto mt-16 pb-20">
						<MyPDFPreview holder={person} settings={settings} />
					</div>
			...
			<div className="grid grid-cols-3 gap-5">
				<div className="col-span-2">
					<IdCardFont person={person} settings={settings} />
					<IdCardBack person={person} settings={settings} />
				</div>
