...
const MyPDFPreview = dynamic(
	() =>
		import(
			"@/features/diplomatic-cards/duplicata/components/id-card-pdf-preview-duplicata"
		),
	{ ssr: false },
);
...
export default function IdCardSectionDuplicate({
	person,
	personDuplicate,
	updateFn,
}: {
	...
	const confirmed = personDuplicate?.documentStage === "confirmed";

	const printed = personDuplicate?.documentStage === "printed";

	const expired = personDuplicate?.expired;
	...
	const markAsPrinted = async () => {
		...
		setLoading(true);
		const res = await updateFn(data, personDuplicate?.id);
		// console.log(res);
		if (res?.status === "error") {
	...
	useEffect(() => {
		if (personDuplicate && data && plateData) {
			const entity = data?.data?.find(
				(e: CardType) => e.name === personDuplicate?.type_card,
			);
			const plaqueChosed = plateData?.data?.find(
				(e: TPlate) => e.code === personDuplicate?.plaque,
			);
			setCardTitle(personDuplicate?.type_card || "");
			setDeliverDate(
				personDuplicate?.issueDate && personDuplicate?.issueDate.length > 0
					? personDuplicate.issueDate
					: new Date().toISOString().split("T")[0],
			);
			setExpirationDate(personDuplicate?.validUntil);
			setPrintColor(personDuplicate?.color || "#AEFCAC");
			if (entity) {
			...
			if (personDuplicate?.observation) {
				setOIText({
					label: personDuplicate?.observation,
					value: personDuplicate?.observation,
				});
			}
		}
	}, [
		...
		<>
			<ResponsiveModal open={openPrintModal}>
				<ResponsiveModalContent className="lg:max-w-6xl [&>button]:hidden overflow-hidden">
					...
					<div className="h-[calc(70%-128px)] overflow-auto mt-16 pb-20">
						<MyPDFPreview holder={person} holderDuplicata={personDuplicate} settings={settings} />
					</div>
			...
			<div className="grid grid-cols-3 gap-5">
				<div className="col-span-2">
					<IdCardFrontRenew person={person} personRenew={personDuplicate} settings={settings} />
					<IdCardBackRenew person={person} personRenew={personDuplicate} settings={settings} />
				</div>
