...
export interface ComboboxProps {
   ...
   **/
  onSearchSync?: (value: string) => IOption[];
...
function removePickedOption(options: IOption[], picked: string | null) {
  if (!picked) return options;
  let cloneOption = [...options];

  cloneOption = cloneOption.filter((val) => picked !== val.value);

  return cloneOption;
}

function isOptionsExist(options: IOption[], targetOption: string) {
  if (options.some((option) => targetOption === option.value)) {
    return true;
  }
...
const ComboboxCreatableSingle = forwardRef<ComboboxRef, ComboboxProps>(
  (
    {
      value,
      onChange,
    ...
    const [selected, setSelected] = React.useState<string | null>(
      arrayOptions?.find((v) => v.value === value)?.label || null
    );
    const [options, setOptions] =
      React.useState<IOption[]>(arrayDefaultOptions);
    ...
    useEffect(() => {
      if (value) {
        setSelected(
          arrayOptions?.find((option) => option.value === value)?.label || null
        );
      }
    }, [value]);
    ...
    const selectables = React.useMemo<IOption[]>(
    ...
    return (
      ...
      >
        <div
          className={cn(
            "min-h-8 rounded-md border border-border text-base md:text-sm ring-offset-background-200 focus-within:ring-2 focus-within:ring-foreground-950 focus-within:ring-offset-2",
            {
              "px-3 py-0": selected,
...
ComboboxCreatableSingle.displayName = "ComboboxCreatableSingle";
export { ComboboxCreatableSingle };
