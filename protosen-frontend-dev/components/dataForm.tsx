...
const DataForm: React.FC<DataFormProps> = ({
	...
	const urlParam = searchParams.has("renew_id")
	...
	async function formAction(formData: FormData) {
		...
		// if (!formData.get("personReplaced")) {
		//   formData.set("personReplaced", "")
		// }
		...
		if (isEdit) {
			actionData = { ...validatedFields.data, id: initialValues["id"] };
		} else {
			actionData = validatedFields.data;
		}

		const { message, errors, data, status } = await onSubmitAction(actionData);

		const id = data?.id || initialValues["id"] || "";

		if (message) {
			if (status === "success") {
				toast.success(message);
				if (!isEdit) {
					router.push(backLink + "/" + id + "/files-form" + urlParam);
				}
	...
	useEffect(() => {
		if (
			...
			initialValues["ownerDiplomaticCardId"]
		) {
			// Va dans data et prends l'objet qui a l'id correspondant à e.target.value
			const holderSelected = holders?.find(
				(holder) => holder.id === initialValues["ownerDiplomaticCardId"],
			);
			setFormValues({
				...formValues,
				holderTitle: holderSelected?.title,
