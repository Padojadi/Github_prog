...
import TableActionsRenew from "./table-actions-renew";
...
const columnFiltersSchema = z.custom<ColumnFiltersState>();
const paginationSchema = z.custom<PaginationState>();

const TableComponentRenew: React.FC<TableComponentProps> = ({
	title,
	headers,
	data,
	actions,
	...
	const columns = React.useMemo<ColumnDef<any, any>[]>(
		() =>
			headers
				.filter(
					(header) =>
						(header.code !== "organism" &&
							header.code !== "previousCard.organism.libelle" &&
							currentUser.isUser) ||
						currentUser.isAdmin ||
					...
					return {
						...
						cell: (info) => {
							if (header.code === "status" || header.code === "documentStage") {
								...
								header.code === "expired" ||
								header.code === "previousCard.expired"
							) {
								return (
									<BadgeStatusComponent
...
export default TableComponentRenew;
