...
const IdCardFrontRenew = ({
	...
	const { printColor, plaque, diplomaticEntity, OIText, cardTitle } =
		usePrintContext();
						...
						<div className="flex gap-1 mt-2">
							...
							<div className="font-bold text-[10px]">
								...
								person.organism?.institutionType === "CONSULAT"
									? `${plaque ? `${plaque?.value}-` : ""}`
									: ""}
								{personRenew?.cardNumber || person?.cardNumber}
							</div>
						</div>
					</div>
				</div>
...
export default IdCardFrontRenew;
