...
const ValidateFormRenewChild = ({
	...
	backLink: string;
	renewData: IPersonCardInfosRenew;
}) => {
				...
				{person?.childDCFiles ? (
					<>
						...
						<ul className="space-y-4">
							{person?.childDCFiles?.passportKey && (
							...
							{person?.childDCFiles?.anKey && (
								<li className="flex items-center gap-2">
									<div className="flex items-center">
										<CheckCircleIcon className="text-green-500 mr-2" />
										<span className="flex-1">Acte de naissance</span>
									</div>
									<Button variant={"link"}>
										<a href={person.anLink} target="_blank">
											Voir document
										</a>
									</Button>
								</li>
							)}
							{person?.childDCFiles?.othersKey && (
								<li className="flex flex-col">
									<div className="flex items-center">
										<CheckCircleIcon className="text-green-500 mr-2" />
										<span className="flex-1">
											{person?.childDCFiles?.othersKey?.length} Autre(s)
											Fichier(s)
										</span>
									</div>
						...
						<div className="mt-6 flex justify-end">
							<form action={formAction}>
								<input type="hidden" name="id" value={renewData.id} />
...
export default ValidateFormRenewChild;
