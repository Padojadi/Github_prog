				...
				{person?.spouseDCFiles ? (
					<>
						...
						<ul className="space-y-4">
							{person?.spouseDCFiles?.passportKey && (
							...
							{person?.spouseDCFiles?.amKey && (
								<li className="flex items-center gap-2">
									...
									<Button variant={"link"}>
										<a href={person.amLink} target="_blank">
											Voir document
										</a>
									</Button>
								</li>
							)}
							{person?.spouseDCFiles?.othersKey && (
								<li className="flex flex-col">
									<div className="flex items-center">
										<CheckCircleIcon className="text-green-500 mr-2" />
										<span className="flex-1">
											{person?.spouseDCFiles?.othersKey?.length} Autre(s)
											Fichier(s)
										</span>
									</div>
...
export default ValidateFormRenewSpouse;
